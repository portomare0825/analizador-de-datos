...
const formatDateDashes = (date: any) => {
    if (!date) return '-';
    let d = date;
...
export const DataTable: React.FC<DataTableProps> = ({ headers, data, originalHeaderMap, hideNotes = false, hideTransactions = false, notesSourceView, useAccountNotes = false, onDataChange, hotelSource, onEditRow, onDeleteRow }) => {
            ...
            try {
                ...
                if (foundData) {
                    setSelectedRow(prev => {
                        ...
                        return {
                            ...foundData, // Campos de la tabla original (fecha_de_llegada, salida, etc)
                            ...prev,      // Mantener lo que ya venía en el row (monto de cxc, etc)
    ...
    if (selectedRow) {
        roomAmount = parseCurrency(selectedRow['Total Hab.']);
        totalAmount = parseCurrency(selectedRow['Total General']);
    ...
    const moneyColumns = ['Total Hab.', 'Monto Pagado', 'Total General', 'Deposito', 'Saldo Pendiente'];
    ...
    const rightAlignedColumns = [
        ...
        'debito', 'credito', 'monto divisa', 'monto bs', 'montodv', 'montobs'
    ];
