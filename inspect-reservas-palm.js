...
async function inspectReservasPalm() {
  const { data, error } = await supabase.from('reservaspalm').select('*').limit(1);
  if (data && data.length > 0) {
    console.log('Columns in reservaspalm:', Object.keys(data[0]));
    console.log('Sample data:', data[0]);
  } else {
    console.log('Table reservaspalm is empty or error:', error?.message);
  }
}
